...
| Change | Where |
| --- | --- |
| The app identifies itself as **BurgerTop**: `APP_NAME` and `ORG` are overridden at startup, so the config directory (`%APPDATA%\BurgerTop`), IPC names, the Windows service (`BurgerTop`), the install directory (`C:\Program Files\BurgerTop`, exe `burgertop.exe`) and every `{$appName}` string follow. English UI strings that referred to RustDesk itself now say BurgerTop. Windows version info and the Android label are renamed too. | `brand_init()` in `src/common.rs`, `src/lang/en.rs`, `Cargo.toml`, `flutter/windows/runner/Runner.rc`, `flutter/android/app/src/main/AndroidManifest.xml`, `flutter/lib/desktop/widgets/tabbar_widget.dart` |
...
Deliberately left as upstream: icons, colors, the Cargo crate name (`rustdesk`, so the build output is `target/release/rustdesk.exe` until packaging renames it), the macOS bundle name (the mac CI steps hardcode `RustDesk.app`), Linux packaging file names (`res/rustdesk.*`, wired into `build.py`), and non-English UI strings. The runtime identity is BurgerTop on every platform regardless.
...
The server defaults and the BurgerTop identity apply on every platform; the driver payload is Windows-only. Linux, macOS, Android and iOS build exactly as upstream RustDesk does, see the [upstream build documentation](https://rustdesk.com/docs/en/dev/build/).

## The Windows driver payload

The program folder carries, next to `burgertop.exe`:
...
- `drivers/RustDeskPrinterDriver/` and `printer_driver_adapter.dll`: the RustDesk v4 printer driver and its adapter. The installer registers the "BurgerTop Printer" on Windows 10 and later; print jobs sent to it arrive on the controlling side.
...
- `src/`: Rust core, connection handling, services (video, audio, input, clipboard, printer) and platform code. `src/common.rs` holds the identity override and the build defaults.
