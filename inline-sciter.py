...
common_tis = open('src/ui/common.tis').read()
...
msgbox = open('src/ui/msgbox.html').read() \
...
def compress(s):
    x = zstd.compress(bytes(s, encoding='utf-8'))
...
with open('src/ui/inline.rs', 'wt') as fh:
    ...
    fh.write('const _MSGBOX: ' + compress(strip(msgbox)) + ';\n')
...
fn get(data: &[u8]) -> String {
    String::from_utf8_lossy(&hbb_common::compress::decompress(data)).to_string()
}
