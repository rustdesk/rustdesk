# Project Status: RustDesk Customization & GCP Integration (2026-06-05)

This document tracks the overall status, progress, active configurations, and milestones for the custom RustDesk project as of June 5, 2026.
...
* Created Inno Setup installer scripts (`RustDesk-Installer.iss`, `RustDesk-Installer-Win7.iss`) to package preconfigured DLLs and configs.
...
* Built a Node.js/TypeScript Express backend in `gcp-api-server`.
...
* Added direct global users (`/api/users`) and peers (`/api/peers`) query endpoints.
* **Shared Address Books Support [NEW]**: Implemented `/api/ab/shared/profiles` with auto-bootstrapping for a global `"Cislink Team"` contacts list. Refactored CRUD endpoint validations to check read/write access permissions on shared entries.
* Containerized the server using a multi-stage `Dockerfile`.
* Verified that compilation (`npm run build`) runs cleanly without errors.
