...
To replace the proprietary RustDesk Server Pro backend, we implemented a custom API server in `gcp-api-server` that routes standard client HTTP calls to Firebase Authentication and Cloud Firestore.
...
* **Document ID**: `guid` (Auto-generated UUID, e.g. `ab_12345` or `ab_cislink_shared_team`)
* **Fields**:
  * `guid`: String (GUID)
  * `name`: String (e.g., "Personal", "Cislink Team")
  * `owner`: String (Associated owner's UID or `"system"`)
  * `rule`: Number (Access level: `3` = full control, `2` = read/write, `1` = read)
  * `is_global`: Boolean (Marks if the address book is shared with all team members)
  * `accessible_users`: Array of Strings (UIDs of users explicitly allowed to access this book)
...
* **Address Book (ab) & Tags**:
  ...
  * `POST /api/ab/shared/profiles` [NEW]: Retrieves shared/global address book profiles, auto-bootstrapping `"Cislink Team"` if missing.
  * `POST /api/ab/peers`: Paginated retrieval of device records inside an address book. Requires read permissions.
  * `POST /api/ab/peer/add/:guid`: Inserts a device and registers its group dynamically. Requires write permissions.
  * `PUT /api/ab/peer/update/:guid`: Modifies device details. Requires write permissions.
  * `DELETE /api/ab/peer/:guid`: Removes multiple device IDs from the book. Requires write permissions.
  * `POST /api/ab/tags/:guid` / `POST /api/ab/tag/add/:guid` / `PUT /api/ab/tag/rename/:guid` / `DELETE /api/ab/tag/:guid`: CRUD tag settings inside the Address Book config. Requires read/write permissions.
