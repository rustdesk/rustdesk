...
const char* uniLinksPrefix = "stardesk://";
                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (!c_args)
  {
    ...
    // std::cout << "StarDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("StarDesk"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"StarDesk - Connection Manager" : L"StarDesk", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }
