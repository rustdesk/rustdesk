...
HICON LoadCustomIcon() {
  static HICON custom_icon_ = nullptr;
...
bool Win32Window::CreateAndShow(const std::wstring& title,
  ...
  // Try to set window icon from data\flutter_assets\assets\icon.ico if it exists
  HICON custom_icon = LoadCustomIcon();
  if (custom_icon != nullptr) {
    SendMessage(window, WM_SETICON, ICON_SMALL, (LPARAM)custom_icon);
    SendMessage(window, WM_SETICON, ICON_BIG, (LPARAM)custom_icon);
  }
