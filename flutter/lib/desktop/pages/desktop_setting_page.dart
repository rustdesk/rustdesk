...
void changeBindInterface(StateSetter parentSetState) async {
  ...
  bool strict = bind.mainGetOptionSync(key: kOptionBindStrict) == 'Y';
  ...
  final isOptFixed =
      isOptionFixed(kOptionBindInterface) || isOptionFixed(kOptionBindStrict);

  gFFI.dialogManager.show((setState, close, context) {
    submit() async {
      await bind.mainSetOption(key: kOptionBindInterface, value: value);
      await bind.mainSetOption(
          key: kOptionBindStrict, value: value.isEmpty || !strict ? '' : 'Y');
      parentSetState(() {});
      close();
    }
            ...
            if (value.isNotEmpty)
              CheckboxListTile(
                ...
                onChanged: isOptFixed
                    ? null
                    : (v) => setState(() => strict = v ?? false),
                title: Text(translate('Strict (no fallback)')),
                subtitle: Text(
                  translate('bind-strict-tip'),
