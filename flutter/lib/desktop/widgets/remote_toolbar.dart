...
class _MonitorMenu extends StatelessWidget {
  ...
  icon() {
    ...
    return Stack(
      ...
      children: [
        ...
        Obx(() {
          RxInt display = CurrentDisplayState.find(id);
          return Text('${display.value == kAllDisplayValue ? 'A' : '${display.value + 1}'}/${pi.displays.length}',
  ...
  openMonitorInTheSameTab(int i, PeerInfo pi) {
    ...
    setRemoteState(() {
      ...
      ffi.ffiModel.pi.currentDisplay = i;
      ffi.ffiModel.updateCurDisplay(ffi.sessionId);
      try {
        CurrentDisplayState.find(id).value = i;
      } catch (e) {
        //
      }
    });
