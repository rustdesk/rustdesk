...
class PlatformFFI {
  ...
  XfinityImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(XfinityImpl xfinityImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = xfinityImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
