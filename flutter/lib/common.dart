...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // techdesk://<connect-id>
    // techdesk://<connect-id>/r
    // techdesk://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing TechDesk...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 TechDesk[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 TechDesk[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or techdesk-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.techdesk.techdesk/macos");
...
Widget loadPowered(BuildContext context) {
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://techdesk.com'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
...
bool whitelistNotEmpty() {
  // https://techdesk.com/docs/en/self-host/client-configuration/advanced-settings/#whitelist
