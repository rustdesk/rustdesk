...
bool shouldSyncAb() {
  return mainGetLocalOption(key: syncAbOption).isNotEmpty;
}

final sortAbTagsOption = 'sync-ab-tags';
bool shouldSortTags() {
  return mainGetLocalOption(key: sortAbTagsOption).isNotEmpty;
}

final filterAbTagOption = 'filter-ab-by-intersection';
bool filterAbTagByIntersection() {
  return mainGetLocalOption(key: filterAbTagOption).isNotEmpty;
}
...
// #region ab
  ...
  Future<void> _pullAb({force = true, quiet = false}) async {
    ...
    try {
      ...
      if (!_everPulledProfiles) {
        ...
        final name = getLocalFlutterOption(k: 'current-ab-name');
  ...
  Future<bool> _getAbSettings() async {
    try {
      final api = "${await mainGetApiServer()}/api/ab/settings";
  ...
  Future<bool> _getPersonalAbGuid() async {
    try {
      final api = "${await mainGetApiServer()}/api/ab/personal";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
      final resp = await http.post(Uri.parse(api), headers: headers);
      if (resp.statusCode == 404) {
  ...
  Future<bool> _getSharedAbProfiles(List<AbProfile> tmpSharedAbs) async {
    final api = "${await mainGetApiServer()}/api/ab/shared/profiles";
    try {
...
// #region sync from recent
    ...
    Future<List<Peer>> getRecentPeers() async {
      try {
        ...
        if (_syncAllFromRecent) {
          ...
          final new_stored_str = await mainGetNewStoredPeers();
        ...
        final loadStr = await mainLoadRecentPeersForAb(
  ...
  void setShouldAsync(bool v) async {
    await mainSetLocalOption(key: syncAbOption, value: v ? 'Y' : '');
    _syncAllFromRecent = true;
    _timerCounter = 0;
  }
...
// #region cache
  _saveCache() {
    try {
      ...
      Map<String, dynamic> m = <String, dynamic>{
        "access_token": mainGetLocalOption(key: 'access_token'),
        "ab_entries": ab_entries,
      };
      mainSaveAb(json: jsonEncode(m));
    } catch (e) {
  ...
  Future<void> loadCache() async {
    try {
      ...
      final access_token = mainGetLocalOption(key: 'access_token');
      if (access_token.isEmpty) return;
      final cache = await mainLoadAb();
...
class LegacyAb extends BaseAb {
  ...
  Future<void> pullAbImpl({force = true, quiet = false}) async {
    final api = "${await mainGetApiServer()}/api/ab";
    int? statusCode;
    try {
...
class Ab extends BaseAb {
  ...
  Future<bool> _fetchPeers(List<Peer> tmpPeers) async {
    final api = "${await mainGetApiServer()}/api/ab/peers";
    try {
      var uri0 = Uri.parse(api);
      final pageSize = 100;
      var total = 0;
  ...
  Future<bool> _fetchTags(List<AbTag> tmpTags) async {
    final api = "${await mainGetApiServer()}/api/ab/tags/${profile.guid}";
    try {
      var uri0 = Uri.parse(api);
      var uri = Uri(
        scheme: uri0.scheme,
...
// #region Peers
  ...
  Future<String?> addPeers(List<Map<String, dynamic>> ps) async {
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/peer/add/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
      for (var p in ps) {
  ...
  Future<bool> changeTagForPeers(List<String> ids, List<dynamic> tags) async {
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/peer/update/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
      var ret = true;
      for (var id in ids) {
  ...
  Future<bool> deletePeers(List<String> ids) async {
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/peer/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
...
// #region Tags
  ...
  Future<bool> addTags(
    ...
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/tag/add/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
  ...
  Future<bool> renameTag(String oldTag, String newTag) async {
    ...
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/tag/rename/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
      final body = jsonEncode({
        "old": oldTag,
  ...
  Future<bool> setTagColor(String tag, Color color) async {
    try {
      final api =
          "${await mainGetApiServer()}/api/ab/tag/update/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
      final body = jsonEncode({
        "name": tag,
  ...
  Future<bool> deleteTag(String tag) async {
    try {
      final api = "${await mainGetApiServer()}/api/ab/tag/${profile.guid}";
      var headers = getHttpHeaders();
      headers['Content-Type'] = "application/json";
