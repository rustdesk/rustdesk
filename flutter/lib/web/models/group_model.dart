...
class GroupModel {
  ...
  Future<bool> _getUsers(List<UserPayload> tmpUsers) async {
    final api = "${await mainGetApiServer()}/api/users";
    try {
  ...
  Future<bool> _getPeers(List<Peer> tmpPeers) async {
    try {
      final api = "${await mainGetApiServer()}/api/peers";
      var uri0 = Uri.parse(api);
      final pageSize = 100;
      var total = 0;
      int current = 0;
  ...
  void _saveCache() {
    try {
      ...
      mainSaveGroup(json: jsonEncode(map));
    } catch (e) {
  ...
  Future<void> loadCache() async {
    try {
      ...
      final cache = await mainLoadGroup();
