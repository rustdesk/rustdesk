...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/RustDesk-Herbin.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"RustDesk-Herbin Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon RustDesk-Herbin.app 200 190 --hide-extension RustDesk-Herbin.app rustdesk-herbin.dmg ./build/macos/Build/Products/Release/RustDesk-Herbin.app")
    os.rename("rustdesk-herbin.dmg", f"../rustdesk-herbin-{version}.dmg")
    '''
    os.chdir("..")
...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/RustDesk-Herbin.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/RustDesk-Herbin.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDesk-Herbin.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDesk-Herbin.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDesk-Herbin.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RustDesk-Herbin.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RustDesk-Herbin.app
    '''.format(pa))
                system2(
                    'create-dmg "RustDesk-Herbin %s.dmg" "target/release/bundle/osx/RustDesk-Herbin.app"' % version)
                os.rename('RustDesk-Herbin %s.dmg' %
                          version, 'rustdesk-herbin-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./rustdesk-herbin-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./rustdesk-herbin-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple rustdesk-herbin-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/RustDesk-Herbin.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
