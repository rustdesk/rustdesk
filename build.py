...
hbb_name = 'marvadesk' + ('.exe' if windows else '')
...
def make_parser():
    ...
    parser.add_argument(
        '--quicksupport',
        action='store_true',
        help='Build cliente QuickSupport: solo recepción, sin ajustes ni conexiones salientes'
    )
    parser.add_argument(
        '--skip-cargo',
        action='store_true',
...
def get_features(args):
    ...
    if getattr(args, 'quicksupport', False):
        features.append('quicksupport')
...
def generate_control_file(version):
    ...
    content = """Package: marvadesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: Soluciones Marva <info@marvadesk.com>
Homepage: https://marvadesk.com
...
Description: MarvaDesk - Soporte remoto.
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/marvadesk')
    system2('mkdir -p tmpdeb/etc/marvadesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/marvadesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/marvadesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/marvadesk/')
    system2(
        'cp ../res/marvadesk.service tmpdeb/usr/share/marvadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/marvadesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/marvadesk.svg')
    system2(
        'cp ../res/marvadesk.desktop tmpdeb/usr/share/applications/marvadesk.desktop')
    system2(
        'cp ../res/marvadesk-link.desktop tmpdeb/usr/share/applications/marvadesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/marvadesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/marvadesk/')
    system2(
        'cp ../res/pam.d/rustdesk.debian tmpdeb/etc/pam.d/marvadesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/marvadesk/files/polkit && chmod a+x tmpdeb/usr/share/marvadesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb marvadesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('marvadesk.deb', '../marvadesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/marvadesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/marvadesk/')
    system2(
        'cp ../res/marvadesk.service tmpdeb/usr/share/marvadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/marvadesk.png')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/MarvaDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"MarvaDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon MarvaDesk.app 200 190 --hide-extension MarvaDesk.app marvadesk.dmg ./build/macos/Build/Products/Release/MarvaDesk.app")
    os.rename("marvadesk.dmg", f"../marvadesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/marvadesk.exe')
    os.chdir('../..')
    if os.path.exists('./marvadesk_portable.exe'):
        ...
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './marvadesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/marvadesk_portable.exe')
    os.rename('./marvadesk_portable.exe', f'./marvadesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/marvadesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/marvadesk.exe')
        system2('mv target/release/marvadesk.exe target/release/MarvaDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\MarvaDesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/MarvaDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/marvadesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/marvadesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/marvadesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv marvadesk-%s-0-x86_64.pkg.tar.zst marvadesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./marvadesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/marvadesk-%s-0.x86_64.rpm ./marvadesk-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/marvadesk-%s-0.x86_64.rpm ./marvadesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/MarvaDesk.app/Contents/MacOS/marvadesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/MarvaDesk.app/Contents/MacOS/')
    ...
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MarvaDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MarvaDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "MarvaDesk %s.dmg" "target/release/bundle/osx/MarvaDesk.app"' % version)
                os.rename('MarvaDesk %s.dmg' %
                          version, 'marvadesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./marvadesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./marvadesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple marvadesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/MarvaDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/marvadesk*.deb ./marvadesk.deb')
                system2('dpkg-deb -R marvadesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/marvadesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/marvadesk-link.desktop tmpdeb/usr/share/applications/marvadesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/marvadesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/marvadesk/')
                os.system('mkdir -p tmpdeb/etc/X11/marvadesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/marvadesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/rustdesk.debian tmpdeb/etc/pam.d/marvadesk')
                system2('strip tmpdeb/usr/bin/marvadesk')
                system2('mkdir -p tmpdeb/usr/share/marvadesk')
                system2('mv tmpdeb/usr/bin/marvadesk tmpdeb/usr/share/marvadesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/marvadesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb marvadesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('marvadesk.deb', 'marvadesk-%s.deb' % version)
