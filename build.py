...
if windows:
    ...
    win_arch = os.environ.get('FLUTTER_WINDOWS_ARCH', win_arch)
...
DRM_PACKAGE_NAME = 'kassatkadesk-unattended-wayland'
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/rustdesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/kassatkadesk/lib/librustdesk.so')
             + glob.glob('tmpdeb/usr/share/kassatkadesk/kassatkadesk')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: kassatkadesk'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: kassatkadesk\nReplaces: kassatkadesk\nProvides: kassatkadesk\n')
...
def build_flutter_deb(version, features):
    ...
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/kassatkadesk.svg')
    system2(
        'cp ../res/rustdesk.desktop tmpdeb/usr/share/applications/kassatkadesk.desktop')
    system2(
        'cp ../res/rustdesk-link.desktop tmpdeb/usr/share/applications/kassatkadesk-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/kassatkadesk/files/polkit && chmod a+x tmpdeb/usr/share/kassatkadesk/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb kassatkadesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('kassatkadesk.deb', '../kassatkadesk-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../kassatkadesk-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/kassatkadesk/lib/librustdesk.so')
                + glob.glob('tmpdeb/usr/share/kassatkadesk/kassatkadesk') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/kassatkadesk/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/kassatkadesk/libdrmtap.so tmpdeb/usr/share/kassatkadesk/libdrmtap.so.0')
        else:
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rustdesk.exe target/release/KassatkaDesk.exe')
        pa = os.environ.get('P')
        if pa:
        ...
        system2(f'mv ../../{res_dir}/kassatkadesk-{version}-win7-install.exe ../..')
    ...
    # verify:  spctl -a -t exec -v /Applications/KassatkaDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
