...
lazy_static::lazy_static! {
    ...
    static ref CONTROL_PERMISSIONS_DISABLE_REMOTE_MODIFY : Arc<Mutex<bool>> = Arc::new(Mutex::new(false));
}
                ...
                tokio::select! {
                    res = c.next() => {
                        match res {
                            ...
                            Ok(Some(ipc::Data::ControlPermissionsDisableRemoteModify(Some(v)))) => {
                                *CONTROL_PERMISSIONS_DISABLE_REMOTE_MODIFY.lock().unwrap() = v;
                            }
                            _ => {}
                        }
                    }
                    ...
                    _ = timer.tick() => {
                        ...
                        c.send(&ipc::Data::ControlPermissionsDisableRemoteModify(None)).await.ok();
                    }
                }
            }
        }
...
pub fn is_remote_modify_disabled_by_control_permissions() -> bool {
    *CONTROL_PERMISSIONS_DISABLE_REMOTE_MODIFY.lock().unwrap()
}
