...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connecting_status", "Подключение к сети TeamDesk..."),
        ...
        ("install_tip", "В некоторых случаях из-за UAC TeamDesk может работать неправильно на удалённом узле. Чтобы избежать возможных проблем с UAC, нажмите кнопку ниже для установки TeamDesk в системе."),
        ("Click to upgrade", "Нажмите, чтобы обновить"),
        ("Configure", "Настроить"),
        ("config_acc", "Чтобы удалённо управлять своим рабочим столом, вы должны предоставить TeamDesk права \"доступа\""),
        ("config_screen", "Для удалённого доступа к рабочему столу вы должны предоставить TeamDesk права \"снимок экрана\""),
        ...
        ("android_input_permission_tip1", "Чтобы удалённое устройство могло управлять вашим Android-устройством с помощью мыши или нажатий, необходимо разрешить TeamDesk использовать службу \"Специальные возможности\"."),
        ("android_input_permission_tip2", "Перейдите на соответствующую страницу системных настроек, найдите и войдите в \"Установленные службы\", включите службу \"TeamDesk Input\"."),
        ...
        ("Turned off", "Отключён"),
        ("Language", "Язык"),
        ("Keep TeamDesk background service", "Держать в фоне службу TeamDesk"),
        ...
        ("Show TeamDesk", "Показать TeamDesk"),
        ...
        ("config_input", "Чтобы управлять удалённым рабочим столом с помощью клавиатуры, необходимо предоставить TeamDesk разрешения \"Мониторинг ввода\"."),
        ("config_microphone", "Чтобы разговаривать с удалённой стороной, необходимо предоставить TeamDesk разрешение \"Запись аудио\"."),
        ...
        ("still_click_uac_tip", "По-прежнему требуется, чтобы удалённый пользователь нажал \"OK\" в окне UAC при запуске TeamDesk."),
        ...
        ("verify_teamdesk_password_tip", "Подтвердить пароль TeamDesk"),
        ...
        ("scam_text1", "Если вы разговариваете по телефону с кем-то, кого вы НЕ ЗНАЕТЕ и НЕ ДОВЕРЯЕТЕ, и он просит вас использовать TeamDesk и запустить его службу, не продолжайте и немедленно прервите разговор."),
        ...
        ("upgrade_teamdesk_server_pro_to_{}_tip", "Обновите TeamDesk Server Pro до версии {} или новее!"),
        ...
        ("selinux_tip", "На вашем устройстве включён SELinux, что может помешать правильной работе TeamDesk на управляемой стороне."),
        ("Change view", "Вид"),
        ...
        ("powered_by_me", "Основано на TeamDesk"),
        ...
        ("allow-only-conn-window-open-tip", "Разрешать подключение только при открытом окне TeamDesk"),
        ...
        ("floating_window_tip", "Помогает поддерживать фоновую службу TeamDesk"),
        ...
        ("upgrade_remote_teamdesk_client_to_{}_tip", "Обновите клиент TeamDesk до версии {} или новее на удалённой стороне!"),
        ...
        ("allow-insecure-tls-fallback-tip", "По умолчанию TeamDesk проверяет сертификат сервера на наличие протоколов, использующих TLS.\nЕсли эта функция включена, TeamDesk пропустит данный этап и продолжит работу в случае неудачной проверки."),
        ("Disable UDP", "Отключить UDP"),
        ("disable-udp-tip", "Определяет, следует ли использовать только TCP.\nЕсли включено, TeamDesk не будет использовать UDP 21116, вместо него будет использоваться TCP 21116."),
        ("server-oss-not-support-tip", "ПРИМЕЧАНИЕ: в OSS-сервере TeamDesk эта функция отсутствует."),
        ...
        ("TeamDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that TeamDesk can use it", "TeamDesk не может получить доступ к сеансу рабочего стола на удалённом устройстве, проверьте, что сеанс запущен и доступен TeamDesk"),
        ...
        ("TeamDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "TeamDesk не смог получить пригодный экран от XDG Desktop Portal, библиотека PipeWire может быть слишком старой"),
        ("TeamDesk could not load a GStreamer component needed for screen capture ({})", "TeamDesk не удалось загрузить компонент GStreamer, необходимый для захвата экрана ({})"),
    ].iter().cloned().collect();
}
