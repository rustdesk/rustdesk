...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Connecting...", "연결 중..."),
        ("Connection in progress. Please wait.", "연결 중입니다. 잠시만 기다려주세요"),
        ...
        ("Failed to connect to rendezvous server", "랑데부 서버에 연결하지 못했습니다"),
        ...
        ("Failed to connect via rendezvous server", "랑데부 서버를 통한 연결에 실패하였습니다"),
        ("Failed to connect via relay server", "릴레이 서버를 통한 연결에 실패하였습니다"),
        ("Failed to make direct connection to remote desktop", "원격 데스크톱에 직접 연결하는 데 실패했습니다"),
        ...
        ("agreement_tip", "설치를 시작하면 라이선스 계약에 수락하는 것입니다."),
        ("Accept and Install", "수락 및 설치"),
        ...
        ("Auto Login", "자동 로그인"),
        ("Enable direct IP access", "다이렉트 IP 연결 사용함"),
        ("Rename", "이름 바꾸기"),
        ("Space", "공백"),
        ...
        ("Closed manually by the peer", "피어에 의해 수동으로 닫힘"),
        ...
        ("whitelist_tip", "화이트리스트에 있는 IP만 액세스 가능"),
        ("Login", "로그인"),
        ("Verify", "확인"),
        ("Remember me", "기억하기"),
        ...
        ("Mouse Drag", "마우스 끌기"),
        ("Three-Finger vertically", "세 손가락으로 세로로"),
        ("Mouse Wheel", "마우스 휠"),
        ("Two-Finger Move", "두 손가락 으로이동"),
        ("Canvas Move", "캔버스 이동"),
        ...
        ("How to get Android input permission?", "Android 입력 권한을 얻는 방법은 무엇인가요?"),
        ...
        ("Start the screen sharing service on boot, requires special permissions", "부팅 시 화면 공유 서비스 시작, 특별한 권한 필요"),
        ...
        ("Show Toolbar", "도구모음 표시"),
        ("Hide Toolbar", "도구모음 숨기기"),
        ...
        ("Full Access", "전체 액세스"),
        ("Screen Share", "화면 공유"),
        ("Wayland requires Ubuntu 21.04 or higher version.", "Wayland에는 Ubuntu 21.04 이상 버전이 필요합니다."),
        ("Wayland requires higher version of linux distro. Please try X11 desktop or change your OS.", "Wayland에는 상위 버전의 Linux 배포판이 필요합니다. X11 데스크톱을 사용하거나 OS를 변경하세요."),
        ("JumpLink", "점프링크"),
        ...
        ("Accept sessions via both", "두 세션 모두 수락"),
        ...
        ("Group", "그룹"),
        ("Search", "검색"),
        ("Closed manually by web console", "웹 콘솔에서 수동으로 닫음"),
        ...
        ("relay_hint_tip", "다이렉트 연결이 불가능할 수 있으며 릴레이를 통해 연결을 시도할 수 있습니다. 또한 첫 번째 시도에서 릴레이를 사용하려면 아이디에 \"/r\" 접미사를 추가하거나 최근 세션 카드에 \"항상 릴레이를 통해 연결\" 옵션이 있는 경우 이 옵션을 선택하면 됩니다."),
        ("Reconnect", "다시 연결"),
        ("Codec", "코덱"),
        ("Resolution", "해상도"),
        ...
        ("os_account_desk_tip", "이 계정은 원격 운영 체제에 로그인하고 헤드리스에서 데스크톱 세션을 활성화하는 데 사용됩니다"),
        ("OS Account", "OS 계정"),
        ...
        ("resolution_fit_local_tip", "로컬 해상도 맞추기"),
        ...
        ("Connection failed due to inactivity", "비활성으로 인해 자동으로 연결이 끊어짐"),
        ("Check for software update on startup", "시작 시 소프트웨어 업데이트 확인"),
        ("upgrade_rustdesk_server_pro_to_{}_tip", "RustDesk Server Pro를 {} 버전 이상으로 업그레이드해 주세요!"),
        ("pull_group_failed_tip", "그룹 새로 고침 실패"),
        ("Filter by intersection", "교차로 필터링"),
        ...
        ("id_input_tip", "ID, 다이렉트 IP 또는 포트가 있는 도메인(<domain>:<port>)을 입력할 수 있습니다.\n다른 서버에 있는 장치에 액세스하려면 서버 주소 (<id>@<server_address>?key=<key_value>)를 추가하세요. 예를들어 \n9123456234@192.168.16.1:21117?key=5Qbwsde3unUcJBtrx9ZkvUmwFNoExHzpryHuPUdqlWM=.\n공용 서버에서 장치에 액세스하려면 공용 서버에 키가 필요하지 않습니다\n첫 번째 연결에서 릴레이 연결을 강제로 사용하려면 ID 끝에 "/r"을 추가합니다. 예를들어 "9123456234/r"."),
        ...
        ("Swap control-command key", "Control 및 Command 키 바꾸기"),
        ("swap-left-right-mouse", "마우스 왼쪽 버튼과 오른쪽 버튼 바꾸기"),
        ("2FA code", "이중 인증 코드"),
        ("More", "더 보기"),
        ("enable-2fa-title", "이중 인증 사용함"),
        ("enable-2fa-desc", "지금 인증기를 설정해 주세요. 휴대폰이나 데스크톱에서 Authy, Microsoft 또는 Google 인증기와 같은 인증기 앱을 사용할 수 있습니다.\n\n앱으로 QR 코드를 스캔하고 앱에 표시된 코드를 입력하면 이중 인증이 가능합니다."),
        ...
        ("outgoing_only_desk_tip", "이것은 맞춤형 에디션입니다.\n다른 장치에 연결할 수는 있지만 다른 장치는 장치에 연결할 수 없습니다."),
        ...
        ("android_new_voice_call_tip", "새로운 음성 통화 요청이 수신되었습니다. 수락하면 오디오가 음성 통신으로 전환됩니다."),
        ...
        ("one-way-file-transfer-tip", "제어된 측에서 단방향 파일 전송이 활성화됩니다."),
        ...
        ("Enable camera", "카메라 보기 사용함"),
        ("No cameras", "카메라 없음"),
        ("d3d_render_tip", "D3D 렌더링이 활성화되면 일부 기기에서는 리모컨 화면이 검은색으로 표시될 수 있습니다."),
        ...
        ("printer-{}-ready-tip", "프린터 {}가 설치되어 사용할 준비가 되었습니다."),
        ...
        ("screenshot-merged-screen-not-supported-tip", "현재 여러 디스플레이의 스크린샷 병합이 지원되지 않습니다. 단일 디스플레이로 전환한 후 다시 시도해 주세요."),
