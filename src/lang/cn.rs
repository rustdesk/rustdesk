...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("multi-control-mode-tip", "由一个控制端持有真实鼠标：其他控制端只移动自己的光标，在点击、滚动、拖拽时临时借用真实鼠标，借用期间可以悬停菜单，先点击一次目标后即可输入；主控一有输入就会夺回鼠标，这会中止协控正在进行的拖拽，但不会撤销它已经产生的效果。两端都需要开启该选项；控制端可在同一页设置“协控操作键”（例如 RControl），按住它即可连续悬停与输入。列出连接的窗口会标出当前持有鼠标的主控，可把主控交给另一个连接，并显示此刻正在操作的是谁；其他控制端的光标不会画在本机桌面上。开启后不支持相对鼠标移动，需要相对鼠标的控制端请改用“每个连接使用独立的鼠标位置”。"),
        ("multi-control-operate-key-label", "协控操作键"),
        ("multi-control-set-primary", "设为主控"),
        ("multi-control-notice-preempted", "操作已被主控接管"),
        ...
        ("multi-control-notice-borrow-expired", "你已经太久没有操作，被控端收回了鼠标"),
        ("multi-control-notice-handover", "主控已换成另一个控制端"),
        ("multi-control-notice-layout-changed", "被控端显示布局已变化，请再移动一次鼠标"),
        ("multi-control-notice-no-permission", "被控端尚未授予你输入权限"),
        ("multi-control-notice-no-position", "请先移动一次鼠标再执行该操作"),
        ("multi-control-notice-no-primary", "被控端当前没有控制端持有鼠标"),
        ("multi-control-notice-not-owner", "只有持有鼠标的控制端才能发送该操作"),
        ("multi-control-notice-peer-unsupported", "被控端未开启该模式，你只能观看"),
        ("multi-control-notice-permission-lost", "被控端已收回你的输入权限"),
        ("multi-control-notice-unsupported", "共享鼠标时该输入方式不可用"),
        ("multi-control-is-primary", "该控制端持有鼠标"),
        ("multi-control-operate-key-invalid", "请输入诸如 RControl 的键名，或单个字母、数字、F1-F24"),
    ].iter().cloned().collect();
}
