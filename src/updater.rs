...
fn check_update(manually: bool) -> ResultType<()> {
    ...
    // Always allow update checks for the KassatkaDesk custom client.
    if !(manually
...
pub fn get_update_download_file_from_url(url: &str) -> Option<PathBuf> {
    ...
    if is_github {
        let trusted_repo = matches!(
            segments.as_slice(),
            ["rustdesk", "rustdesk", "releases", "download", tag, _] if !tag.is_empty()
        ) || matches!(
            segments.as_slice(),
            ["ghsergey", "KassatkaDesk", "releases", "download", tag, _] if !tag.is_empty()
        );
        if !trusted_repo {
            return None;
        }
    } else if !matches!(segments.as_slice(), ["releases", .., _]) {
        return None;
    }
...
pub fn check_update_as_root() -> ResultType<bool> {
    ...
    let dmg_url = format!("{}/kassatkadesk-{}-{}.dmg", download_url, version, arch);
    ...
    let filename = dmg_url
        .split('/')
        .last()
        .unwrap_or("kassatkadesk.dmg");
...
mod tests {
    ...
    fn update_download_file_accepts_expected_github_asset_urls() {
        ...
        let custom_file = get_download_file_from_url(
            "https://github.com/ghsergey/KassatkaDesk/releases/download/1.5.0/kassatkadesk-1.5.0-x86_64.dmg",
        )
        .expect("valid KassatkaDesk GitHub release asset URL");
        assert_eq!(
            custom_file.file_name().and_then(|name| name.to_str()),
            Some("kassatkadesk-1.5.0-x86_64.dmg")
        );
        ...
        assert_eq!(
            filevault_file.file_name().and_then(|name| name.to_str()),
            Some("kassatkadesk-1.5.0-x86_64.dmg")
        );
    }
