...
enum PasswordSource {
    PersonalAbHash(Vec<u8>),
    PersonalAbPassword(String),
    SharedAb(String),
    UiLogin(String),
    PeerConfig(Vec<u8>),
    Undefined,
...
impl PasswordSource {
    pub fn is_personal_ab_hash(&self, password: &[u8]) -> bool {
        if password.is_empty() {
            return false;
        }
        match self {
            PasswordSource::PersonalAbHash(p) => p == password,
            _ => false,
        }
    }

    pub fn is_personal_ab_password(&self, password: &[u8], hash: &Hash) -> bool {
        if password.is_empty() {
            return false;
        }
        match self {
            PasswordSource::PersonalAbPassword(p) => Self::equal(p, password, hash),
            _ => false,
        }
    }
    ...
    pub fn is_ui_login(&self, password: &[u8], hash: &Hash) -> Option<String> {
        ...
        match self {
            PasswordSource::UiLogin(p) => {
                if Self::equal(p, password, hash) {
                    Some(p.clone())
                } else {
                    None
                }
...
pub struct LoginConfigHandler {
    ...
    shared_password: Option<String>, // whether the preset password is shared password
}
...
impl LoginConfigHandler {
    ...
    pub fn handle_peer_info(&mut self, pi: &PeerInfo) {
        ...
        if remember {
            ...
            if self.password_source.is_personal_ab_hash(&password)
                || self
                    .password_source
                    .is_personal_ab_password(&password, &hash)
            {
                config.password = password.clone();
        ...
        #[cfg(feature = "flutter")]
        {
            if !config.password.is_empty()
                && !self.password_source.is_shared_ab(&password, &hash)
                && !self.password_source.is_personal_ab_hash(&password)
            {
            ...
            if remember {
                if let Some(password) = self.password_source.is_ui_login(&password, &hash) {
                    let evt: HashMap<&str, String> = HashMap::from([
                        ("name", "sync_peer_password_to_personal_ab".to_string()),
                        ("id", self.id.clone()),
                        ("password", password),
                    ]);
                    let evt = serde_json::ser::to_string(&evt).unwrap_or("".to_owned());
...
#[inline]
fn try_get_password_from_personal_ab(
    lc: Arc<RwLock<LoginConfigHandler>>,
    password: &mut Vec<u8>,
    hash: &Hash,
) {
                ...
                if !p.password.is_empty() {
                    ...
                    *password = res[..].into();
                    lc.write().unwrap().password_source =
                        PasswordSource::PersonalAbPassword(p.password.clone());
                } else if let Ok(hash_password) =
                    base64::decode(p.hash.clone(), base64::Variant::Original)
                {
                    if !hash_password.is_empty() {
                        *password = hash_password.clone();
                        lc.write().unwrap().password_source =
                            PasswordSource::PersonalAbHash(hash_password);
                    }
                }
            }
        }
...
pub async fn handle_login_from_ui(
    ...
    let mut hash_password = if password.is_empty() {
        ...
        if password2.is_empty() {
            ...
            if !password2.is_empty() {
                lc.write().unwrap().password_source = PasswordSource::PeerConfig(password2.clone());
            }
        }
        password2
    } else {
        lc.write().unwrap().password_source = PasswordSource::UiLogin(password.clone());
        let mut hasher = Sha256::new();
        hasher.update(password);
