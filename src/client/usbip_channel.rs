    ...
    match union {
        Some(Union::PushResult(r)) => {
            ...
            if !crate::client::usbip_attach::is_valid_bus_id(&r.bus_id)
                || !handler.usb.share_owned(&r.bus_id)
            {
                log::warn!(
        ...
        Some(Union::Opened(o)) if o.channel_id >= 0 => forward(
            handler,
            o.channel_id,
        ...
        Some(Union::Data(d)) if d.channel_id >= 0 => forward(
            handler,
            d.channel_id,
            crate::client::usbip_attach::Inbound::Data(d.data),
        ),
        Some(Union::Close(c)) if c.channel_id >= 0 => forward(
            handler,
            c.channel_id,
            crate::client::usbip_attach::Inbound::Closed,
        ),
        Some(Union::Data(d)) => {
            if handler.usb.share_send(d.channel_id, d.data) {
                notify_closed(handler, d.channel_id);
            }
        }
        Some(Union::WindowUpdate(w)) => handler.usb.window_update(w.channel_id, w.add),
        Some(Union::Close(c)) => {
            log::info!("usb push: peer closed channel {}", c.channel_id);
            handler.usb.unregister_share_channel(c.channel_id);
        }
        ...
        Some(Union::Open(open)) if open.channel_id < 0 => {
            ...
            if handler.usb.share_channel_live(id) {
                return;
            }
            ...
            if !handler.usb.share_pending_take(&open.bus_id) {
                log::warn!(
            ...
            handler.usb.register_share_channel(id, open.bus_id.clone(), tx, flow.clone());
...
fn forward(handler: &FlutterHandler, channel_id: i32, msg: crate::client::usbip_attach::Inbound) {
    if handler.usb.forward_send(channel_id, msg) {
        notify_closed(handler, channel_id);
    }
}

/// Tells the peer we've abandoned a channel it doesn't yet know is dead --
/// without this its own side keeps the relay task/entry alive until the
/// whole session ends.
fn notify_closed(handler: &FlutterHandler, channel_id: i32) {
    match handler.any_session() {
        Some(session) => session.usb_close_forward(channel_id),
        None => log::warn!(
            "usb: dropped channel {} but no session to notify the peer",
            channel_id
        ),
    }
}
