on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set unload_legacy_service to "launchctl bootout system " & quoted form of legacy_daemon_plist & " 2>/dev/null || launchctl unload -w " & quoted form of legacy_daemon_plist & " || true;"
  ...
  set unload_bad_service to "launchctl bootout system " & quoted form of bad_daemon_plist & " 2>/dev/null || launchctl unload -w " & quoted form of bad_daemon_plist & " || true;"
  set remove_legacy_plists to "rm -f " & quoted form of legacy_agent_plist & " " & quoted form of legacy_daemon_plist & ";"
  set remove_bad_plists to "rm -f " & quoted form of bad_agent_plist & " " & quoted form of bad_daemon_plist & ";"
  ...
  set sh to "set -e;" & check_source & resolve_uid & unload_agent & unload_service & unload_legacy_agent & unload_legacy_service & unload_bad_agent & unload_bad_service & remove_legacy_plists & remove_bad_plists & kill_others & copy_files & write_daemon_plist & write_agent_plist & load_service & load_agent
