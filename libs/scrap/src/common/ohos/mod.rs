...
compile_error!(
    "OHOS uses its AVCodec backend; desktop `hwcodec`, `mediacodec`, and `vram` features are unsupported."
);

pub mod avcodec;
pub(crate) mod codec_adapter;
pub mod direct_render;

pub use codec_adapter::{encoder_config, retry_encoder_setup};
pub use direct_render::{
    enter_direct_render_context, lookup_direct_render_target, register_direct_render_target_lookup,
