...
use hbb_common::{bail, config, platform::linux::CMD_SH, ResultType};
...
pub fn fill_displays(
    ...
    let has_postion_attr = HAS_POSITION_ATTR.load(Ordering::SeqCst);
...
fn compare_left_up_cornor(w: usize, d1: &[u8], d2: &[u8]) -> bool {
    if w == 0 {
        return false;
    }
    if d1.len() != d2.len() {
    ...
    let threshold = std::cmp::max(8, total_px / 8);
...
fn fill_multi_matched_positions_cursor(
    ...
    let mut mached_indices = Vec::new();
    ...
    for idx in multi_matched_indices {
        match (
            ...
            (Some(crate::Display::WAYLAND(d)), Some(pw_stream), Some(pw_stream_with_cursor)) => {
                ...
                let matched_len = mached_indices.len();
                for (i, wd) in displays.displays.iter().enumerate() {
                    if mached_indices.contains(&i) {
                        continue;
                    }
                    ...
                    {
                        for i in 0..3 {
                            mouse_move_to(wd.x + 8 + i, wd.y + 8 + i);
                            std::thread::sleep(std::time::Duration::from_millis(50));
                        }
                        rec.saved_raw_data.clear();
                        match rec.capture(CAPTURE_TIMEOUT_MS) {
                            Ok(PixelProvider::BGR0(_, _, data2)) if is_bgr => {
                                if compare_left_up_cornor(w, &first_buf, data2) {
                                    d.0.position = (wd.x, wd.y);
                                    pw_stream.position = (wd.x, wd.y);
                                    mached_indices.push(i);
                                    debug!(
                ...
                if matched_len == mached_indices.len() {
                    error!(
    ...
    for (x, y) in display_order.into_iter() {
        for i in 0..streams.len() {
            if streams[i].position.0 == x && streams[i].position.1 == y {
                ...
                // shared_displays.len() mast be equal to streams.len()
