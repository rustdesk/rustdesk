...
async fn kde_show_desktop() -> ResultType<()> {
    ...
    proxy
        .call_method("invokeShortcut", &("Show Desktop",))
        .await?;
    Ok(())
}
...
fn fill_multi_matched_positions(
    ...
    // It's ok to spawn a new thread here, because it is not called frequently.
    std::thread::spawn(|| kde_show_desktop().ok()).join().ok();
