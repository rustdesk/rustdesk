...
pub static FORCE_SERVICE_CONFIG: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
...
fn patch(path: PathBuf) -> PathBuf {
    if let Some(_tmp) = path.to_str() {
        #[cfg(windows)]
        {
            if FORCE_SERVICE_CONFIG.load(std::sync::atomic::Ordering::SeqCst) || _tmp.contains("system32\\config\\systemprofile") {
                let windir = std::env::var("windir").unwrap_or_else(|_| "C:\\Windows".to_owned());
                return format!("{}\\ServiceProfiles\\LocalService\\AppData\\Roaming\\{}", windir, *APP_NAME.read().unwrap()).into();
            }
            return _tmp.into();
        }
        #[cfg(target_os = "macos")]
...
impl Config {
    ...
    pub fn verify_permanent_password(pass: &str) -> bool {
        let config = Config::load_::<Config>("");
        let (decrypted, _, _) = decrypt_str_or_original(&config.password, PASSWORD_ENC_VERSION);
        decrypted == pass
    }
